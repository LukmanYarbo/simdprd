...
<x-breadcrumbs :items="[
    ...
    ['label' => 'Edit Anggota', 'icon' => 'ti ti-pencil']
]" />
@endsection
...
<div class="container-fluid">
    ...
    <form id="wizardForm" action="{{ route('admin.anggota.update', $anggota) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                ...
                <div class="card-body p-4 pt-0">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('nokk') is-invalid @enderror" id="nokk" name="nokk" value="{{ old('nokk', $anggota->nokk) }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('tempat_lahir') is-invalid @enderror" id="tempat_lahir" name="tempat_lahir" value="{{ old('tempat_lahir', $anggota->tempat_lahir) }}" required>
                            @error('tempat_lahir')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6">
                            <label for="tgl_lahir" class="form-label">Tanggal Lahir</label>
                            <input type="date" class="form-control @error('tgl_lahir') is-invalid @enderror" id="tgl_lahir" name="tgl_lahir" value="{{ old('tgl_lahir', $anggota->tgl_lahir ? $anggota->tgl_lahir->format('Y-m-d') : '') }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="btn-group w-100" role="group">
                                <input type="radio" class="btn-check" name="jk" id="jk_l" value="L" {{ old('jk', $anggota->jk) == 'L' ? 'checked' : '' }} required>
                                <label class="btn btn-outline-primary" for="jk_l"><i class="ti ti-gender-male me-1"></i> Laki-laki</label>
                                
                                <input type="radio" class="btn-check" name="jk" id="jk_p" value="P" {{ old('jk', $anggota->jk) == 'P' ? 'checked' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('id_agama') is-invalid @enderror" id="id_agama" name="id_agama" required>
                                @foreach($agamas as $agama)
                                    <option value="{{ $agama->id }}" {{ old('id_agama', $anggota->id_agama) == $agama->id ? 'selected' : '' }}>{{ $agama->nama }}</option>
                                @endforeach
                            </select>
                ...
                <div class="card-body p-4 pt-0">
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <textarea class="form-control @error('alamat_lengkap') is-invalid @enderror" id="alamat_lengkap" name="alamat_lengkap" rows="3" required>{{ old('alamat_lengkap', $anggota->alamat_lengkap) }}</textarea>
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <select class="form-select @error('id_skpd') is-invalid @enderror" id="id_skpd" name="id_skpd">
                                <option value="" disabled>Pilih SKPD</option>
                                @foreach($skpds as $skpd)
                                    <option value="{{ $skpd->id }}" {{ old('id_skpd', $anggota->id_skpd) == $skpd->id ? 'selected' : '' }}>{{ $skpd->namaskpd }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('id_status_keanggotaan') is-invalid @enderror" id="id_status_keanggotaan" name="id_status_keanggotaan" required>
                                @foreach($statusKeanggotaans as $sk)
                                    <option value="{{ $sk->id }}" {{ old('id_status_keanggotaan', $anggota->id_status_keanggotaan) == $sk->id ? 'selected' : '' }}>{{ $sk->nama }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('id_dprd') is-invalid @enderror" id="id_dprd" name="id_dprd" required>
                                @foreach($jabatans as $j)
                                    <option value="{{ $j->id }}" {{ old('id_dprd', $anggota->id_dprd) == $j->id ? 'selected' : '' }}>{{ $j->nama }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="date" class="form-control @error('tgl_mulai') is-invalid @enderror" id="tgl_mulai" name="tgl_mulai" value="{{ old('tgl_mulai', $anggota->tgl_mulai ? $anggota->tgl_mulai->format('Y-m-d') : '') }}" required>
                        ...
                        <div class="col-md-4 text-center">
                            ...
                            <div class="mb-3">
                                @if($anggota->foto_anggota)
                                    <img id="preview" src="{{ asset('storage/' . $anggota->foto_anggota) }}" class="rounded border shadow-sm" width="180" height="180" title="Klik untuk ganti foto">
                                @else
                                    <img id="preview" src="https://ui-avatars.com/api/?name={{ $anggota->nama_anggota }}&background=random&size=180" class="rounded border shadow-sm" width="180" height="180" title="Klik untuk ganti foto">
                                @endif
                            </div>
                            <input type="file" class="form-control form-control-sm" id="foto_anggota" name="foto_anggota" onchange="previewImage(event)">
                        </div>
                        <div class="col-md-8">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <div class="d-flex gap-3 mt-2">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="status_bpjs" id="status_bpjs_y" value="Y" {{ old('status_bpjs', $anggota->status_bpjs) == 'Y' ? 'checked' : '' }}>
                                            <label class="form-check-label" for="status_bpjs_y">Ya</label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="status_bpjs" id="status_bpjs_t" value="T" {{ old('status_bpjs', $anggota->status_bpjs) == 'T' ? 'checked' : '' }}>
                                ...
                                <div class="col-md-6" id="no_bpjs_wrapper" style="{{ old('status_bpjs', $anggota->status_bpjs) == 'Y' ? '' : 'visibility: hidden; pointer-events: none;' }}">
                                    <label for="no_bpjs" class="form-label">No. BPJS</label>
                                    <input type="text" class="form-control @error('no_bpjs') is-invalid @enderror" id="no_bpjs" name="no_bpjs" value="{{ old('no_bpjs', $anggota->no_bpjs) }}">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="d-flex gap-3 mt-2">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="status_jkk" id="status_jkk_y" value="Y" {{ old('status_jkk', $anggota->status_jkk) == 'Y' ? 'checked' : '' }}>
                                            <label class="form-check-label" for="status_jkk_y">Ya</label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="status_jkk" id="status_jkk_t" value="T" {{ old('status_jkk', $anggota->status_jkk) == 'T' ? 'checked' : '' }}>
                                ...
                                <div class="col-md-6" id="no_jkk_wrapper" style="{{ old('status_jkk', $anggota->status_jkk) == 'Y' ? '' : 'visibility: hidden; pointer-events: none;' }}">
                                    <label for="no_jkk" class="form-label">No. JKK</label>
                                    <input type="text" class="form-control @error('no_jkk') is-invalid @enderror" id="no_jkk" name="no_jkk" value="{{ old('no_jkk', $anggota->no_jkk) }}">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="d-flex gap-3 mt-2">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="status_jkm" id="status_jkm_y" value="Y" {{ old('status_jkm', $anggota->status_jkm) == 'Y' ? 'checked' : '' }}>
                                            <label class="form-check-label" for="status_jkm_y">Ya</label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="status_jkm" id="status_jkm_t" value="T" {{ old('status_jkm', $anggota->status_jkm) == 'T' ? 'checked' : '' }}>
                                ...
                                <div class="col-md-6" id="no_jkm_wrapper" style="{{ old('status_jkm', $anggota->status_jkm) == 'Y' ? '' : 'visibility: hidden; pointer-events: none;' }}">
                                    <label for="no_jkm" class="form-label">No. JKM</label>
                                    <input type="text" class="form-control @error('no_jkm') is-invalid @enderror" id="no_jkm" name="no_jkm" value="{{ old('no_jkm', $anggota->no_jkm) }}">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select @error('status_tjgn_perum') is-invalid @enderror" id="status_tjgn_perum" name="status_tjgn_perum" required>
                                        <option value="T" {{ old('status_tjgn_perum', $anggota->status_tjgn_perum) == 'T' ? 'selected' : '' }}>Tidak</option>
                                        <option value="Y" {{ old('status_tjgn_perum', $anggota->status_tjgn_perum) == 'Y' ? 'selected' : '' }}>Ya</option>
                                    </select>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select @error('status_tjgn_transport') is-invalid @enderror" id="status_tjgn_transport" name="status_tjgn_transport" required>
                                        <option value="T" {{ old('status_tjgn_transport', $anggota->status_tjgn_transport) == 'T' ? 'selected' : '' }}>Tidak</option>
                                        <option value="Y" {{ old('status_tjgn_transport', $anggota->status_tjgn_transport) == 'Y' ? 'selected' : '' }}>Ya</option>
                                    </select>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        new AnggotaWizard({
            ...
            currentData: {
                id: "{{ $anggota->id }}",
                prov: @json(old('prov', $anggota->prov)),
                kab: @json(old('kab', $anggota->kab)),
                kec: @json(old('kec', $anggota->kec)),
                desa: @json(old('desa', $anggota->desa))
            }
        });
    });
</script>
